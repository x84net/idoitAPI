...
def create_panel_1ru_24x_rj45_new(pan_title, prefix_in, in_start_nr, prefix_out, out_start_nr):
    ...
    # build list of 24 ports that should be created - IN port: <P>xx, OUT port: <P>xx
    catg_lst = []
    for i in range(24):
        catg_lst.append({'connection_type': 'C__CONNECTION_TYPE__RJ45',
    ...
    # build list of port siblings - build batch request
    ...
    # send batch request / list of siblings to idoit - second API call
    res_d, lst_q, dct_q = pyDoit.send_batch()

    log.info("IN/OUT Ports connected to each other:\n{}".format(json.dumps(res_d, indent=4, sort_keys=True)))
    return resPan['id']
...
if __name__ == "__main__":
    ...
    log.info("ObjID of new Panel is: {} - https://demo.i-doit.com/?objID={}".format(res, res))
